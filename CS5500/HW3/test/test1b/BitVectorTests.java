...
import q1b.BitVector;
import q1b.IBitVector;
import q1b.Iterator;
...
public class BitVectorTests {
	...
	/**
	 * A method to test the iterator functionality of the BitVector class
	 * 
	 * The cases tested are: 
	 * 1) Correct values are returned by the iterator
	 * 2) The correct number of values are returned
	 * 3) Supports large numbers and non-uniformly distributed numbers
	 */
	@Test
	public void testIterator() {
		...
		while(it.hasAnotherElement()) {
			Integer num = it.nextElement();
			nums.add(num);
		}
		assertEquals("Incorrect number of values returned", nums.size(), vector.size());
		assertThat(nums, CoreMatchers.hasItems(1,4,45));
		
		IBitVector vector2 = new BitVector();
		...
		while(it2.hasAnotherElement()) {
			Integer num = it2.nextElement();
			nums2.add(num);
		}
		assertEquals("Incorrect number of values returned", nums2.size(), vector2.size());
		assertThat(nums2, CoreMatchers.hasItems(100,44202,4531231,3,355));
