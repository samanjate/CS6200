...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        varDomain = variableDomainsDict[eliminationVariable]
...
def normalize(factor):
    ...
    for fact in factor.unconditionedVariables():
        if len(variableDomainsDict[fact]) == 1:
            conditioned.add(fact)
    ...
    for assignment in newFactor.getAllPossibleAssignmentDicts():
        prob = factor.getProbability(assignment)
        newFactor.setProbability(assignment, prob/sumProb)
    return newFactor
