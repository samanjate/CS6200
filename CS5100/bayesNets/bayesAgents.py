...
def constructBayesNet(gameState):
    ...
    for var in obsVars:
        ...
        variableDomainsDict[var] = set(OBS_VALS)
...
def fillYCPT(bayesNet, gameState):
    ...
    """
    from layout import PROB_BOTH_TOP, PROB_ONLY_LEFT_TOP, PROB_BOTH_BOTTOM, PROB_ONLY_LEFT_BOTTOM
...
def fillObsCPT(bayesNet, gameState):
    ...
    """
    HOUSE_VALUES = [BOTTOM_LEFT_VAL, TOP_LEFT_VAL, BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL]
    counter = 0
    
    for pos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(pos):
            obsPos = OBS_VAR_TEMPLATE % obsPos
            obsVarFactor = bn.Factor([obsPos], HOUSE_VARS, bayesNet.variableDomainsDict())             
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        query = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        tempLeftFoodLeft = 0
        tempRightFoodLeft = 0
        for assignment in query.getAllPossibleAssignmentDicts():
            prob = query.getProbability(assignment)
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                tempLeftFoodLeft = prob
            elif assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                tempRightFoodLeft = prob
        leftExpectedValue = tempLeftFoodLeft * WON_GAME_REWARD + (1  - tempLeftFoodLeft) * GHOST_COLLISION_REWARD
        rightExpectedValue = tempRightFoodLeft * WON_GAME_REWARD + (1 - tempRightFoodLeft) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for outcome in self.getExplorationProbsAndOutcomes(evidence):
            evidence.update(outcome[1])
            vals = self.computeEnterValues(evidence, enterEliminationOrder)
            expectedValue += max(vals[0], vals[1]) * outcome[0]
